...
namespace TestJwt.Scripts;

public class TokenService
{
    private const int AccessTokenLifetimeMinute = 10;
    private const int RefreshTokenLifetimeDay = 7;
    ...
    public static bool GetTwtTokenRevoked(string refreshToken)
    {
        return RefreshTokens.FirstOrDefault(t => t.Jti == GetJwtTokenData(refreshToken).Jti).Revoke;
    }

    public static void RevokeJwtToken(string refreshToken)
    {
        var token = RefreshTokens.FirstOrDefault(t => t.Jti == GetJwtTokenData(refreshToken).Jti);
        token.Revoke = true;
    }
    ...
    public static bool CheckRefreshToken(string refreshToken, int passwordVersion)
    {
